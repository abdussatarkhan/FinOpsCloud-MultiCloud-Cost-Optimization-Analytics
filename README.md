...
# 📊 FinOpsCloud: Enterprise Multi-Cloud Cost & FinOps Intelligence

**Enterprise Multi-Cloud (AWS/GCP/Azure) FinOps Data Warehouse: Unit economics per API transaction, Kubernetes pod rightsizing, RI/Savings Plan coverage, and unattached resource waste.**

[![CI Analytics Pipeline](https://github.com/abdussatarkhan/FinOpsCloud-MultiCloud-Cost-Optimization-Analytics/actions/workflows/ci.yml/badge.svg)](https://github.com/abdussatarkhan/FinOpsCloud-MultiCloud-Cost-Optimization-Analytics/actions)
...
[![Maintenance](https://img.shields.io/badge/Maintained%3F-yes-brightgreen.svg?style=flat-square)](https://github.com/abdussatarkhan/FinOpsCloud-MultiCloud-Cost-Optimization-Analytics)
...
**FinOpsCloud-MultiCloud-Cost-Optimization-Analytics** is an enterprise-grade analytics data warehouse and business intelligence platform built for **Cloud Infrastructure & FinOps Optimization**.
...
### 1️⃣ Executive KPI Command Center (Cloud Modern Light Slate (Light))
...
```
                    ...
                    └────────────┬────────────┘
                                 ...
                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_cloud_accounts │──►│     fact_daily_cloud_spend │◄──┤     dim_services     │
└──────────────────────┘   └─────────────┬───────────┘   └─────────────────────────┘
                                         │
       ┌─────────────────────────────────┼─────────────────────────────────┐
       ▼                                 ▼                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_k8s_clusters │   │     dim_cost_centers    │   │     dim_date         │
└──────────────────────┘   └─────────────────────────┘   └─────────────────────────┘
```
...
FROM finops_dw.fact_daily_cloud_spend f
JOIN finops_dw.dim_date dt ON f.date_key = dt.date_key
...
```bash
git clone https://github.com/abdussatarkhan/FinOpsCloud-MultiCloud-Cost-Optimization-Analytics.git
cd FinOpsCloud-MultiCloud-Cost-Optimization-Analytics
